...
ko: ## Run Ko
	...
	KO_DOCKER_REPO=$(KO_PREFIX)/timestamp-server ko build --bare \
		...
		--image-refs timestampServerImagerefs github.com/navzar05/descentralized-timestamp-authority/cmd/timestamp-server

	# timestamp-cli
	LDFLAGS="$(LDFLAGS)" GIT_HASH=$(GIT_HASH) GIT_VERSION=$(GIT_VERSION) \
	KO_DOCKER_REPO=$(KO_PREFIX)/timestamp-cli ko build --bare \
		--platform=all --tags $(GIT_VERSION) --tags $(GIT_HASH) \
		--image-refs timestampCLIImagerefs github.com/navzar05/descentralized-timestamp-authority/cmd/timestamp-cli
...
ko-local: ## Run Ko locally
	...
	ko publish --base-import-paths \
		...
		github.com/navzar05/descentralized-timestamp-authority/cmd/timestamp-server

	LDFLAGS="$(CLI_LDFLAGS)" GIT_HASH=$(GIT_HASH) GIT_VERSION=$(GIT_VERSION) \
	ko publish --base-import-paths \
		--tags $(GIT_VERSION) --tags $(GIT_HASH) --local \
		github.com/navzar05/descentralized-timestamp-authority/cmd/timestamp-cli
